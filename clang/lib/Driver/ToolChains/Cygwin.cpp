...
void tools::Cygwin::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (!D.SysRoot.empty()) {
    ...
    const auto Dir = D.SysRoot + "/lib/w32api";
    if (VFS.exists(Dir)) {
      CmdArgs.push_back("-L");
      CmdArgs.push_back(Args.MakeArgString(Dir));
    }
  } else {
    if (std::string InstallBase{llvm::sys::path::parent_path(D.Dir)};
        InstallBase != "/usr") {
      CmdArgs.push_back("-L");
      CmdArgs.push_back(Args.MakeArgString(InstallBase + "/lib"));
    }
    if (IsLLD) {
      for (auto Dir : {"/usr/lib", "/usr/lib/w32api"}) {
        if (VFS.exists(Dir)) {
          CmdArgs.push_back("-L");
          CmdArgs.push_back(Args.MakeArgString(Dir));
        }
  ...
  for (auto Sym : {"_Znwm", "_Znam", "_ZdlPv", "_ZdaPv", "_ZnwmRKSt9nothrow_t",
...
void toolchains::Cygwin::findGccLibDir(const llvm::Triple &LiteralTriple) {
  ...
  for (StringRef CandidateLib : {"lib", "usr/lib"}) {
...
findClangRelativeSysroot(const Driver &D, const llvm::Triple &LiteralTriple,
  ...
  const auto ClangRoot = llvm::sys::path::parent_path(D.Dir) + "/";
  for (StringRef CandidateSubdir : Subdirs) {
    if (llvm::sys::fs::is_directory(ClangRoot + CandidateSubdir)) {
      SubdirName = CandidateSubdir;
      return (ClangRoot + CandidateSubdir).str();
    }
  }
  return make_error_code(std::errc::no_such_file_or_directory);
}
...
static bool looksLikeCygwinSysroot(const std::string &Directory) {
  if (!llvm::sys::fs::exists(Directory + "/include/cygwin/version.h"))
    return false;
  // On cygwin, /usr/lib is not present on real filesystem so should search
  // ../lib too.
  if (!llvm::sys::fs::exists(Directory + "/lib/libcygwin.a") &&
      !llvm::sys::fs::exists(Directory + "/../lib/libcygwin.a"))
    return false;
  return true;
}
...
void toolchains::Cygwin::AddClangSystemIncludeArgs(
  ...
  if (!::isCrossCompiling(getTriple(), /*RequireArchMatch=*/false) ||
      getDriver().SysRoot.size()) {
    addSystemInclude(DriverArgs, CC1Args, Base + "local/include");
    addSystemInclude(DriverArgs, CC1Args, Base + "include");
    addSystemInclude(DriverArgs, CC1Args, Base + "include/w32api");
  }
}
...
void toolchains::Cygwin::AddClangCXXStdlibIncludeArgs(
  ...
  auto &VFS = getDriver().getVFS();
  llvm::SmallVector<llvm::SmallString<1024>, 5> CppIncludeBases;
  switch (GetCXXStdlibType(DriverArgs)) {
  case ToolChain::CST_Libcxx:
    if (std::string InstallBase{llvm::sys::path::parent_path(getDriver().Dir)};
        InstallBase != Base) {
      append(CppIncludeBases.emplace_back(InstallBase), "include/c++/v1");
    }
    append(CppIncludeBases.emplace_back(Base), "include", TripleDirName,
           "c++/v1");
    append(CppIncludeBases.emplace_back(Base), "include/c++/v1");
    if (getDriver().SysRoot.size()) {
      append(CppIncludeBases.emplace_back(Base), "usr/include", TripleDirName,
             "c++/v1");
      append(CppIncludeBases.emplace_back(Base), "usr/include/c++/v1");
    }
    for (auto &CppIncludeBase : CppIncludeBases) {
      if (VFS.exists(CppIncludeBase))
        addSystemInclude(DriverArgs, CC1Args, CppIncludeBase);
    }
    break;

  case ToolChain::CST_Libstdcxx:
    // Cygwin distribution places:
    //  $sysroot/lib/gcc/$triple/$version/include/c++
    //  $sysroot/lib/gcc/$triple/$version/include/c++/backward
    //  $sysroot/lib/gcc/$triple/$version/include/c++/$triple
    // MSYS2 distribution places:
    //  $sysroot/include/$triple/c++/$version
    //  $sysroot/include/$triple/c++/$version/backward
    //  $sysroot/include/$triple/c++/$version/$triple
    llvm::sys::path::append(CppIncludeBases.emplace_back(Base), "lib/gcc",
                            TripleDirName, Ver, "include/c++");
    llvm::sys::path::append(CppIncludeBases.emplace_back(Base), "include",
                            TripleDirName, "c++", Ver);
    if (getDriver().SysRoot.size()) {
      llvm::sys::path::append(CppIncludeBases.emplace_back(Base), "usr/lib/gcc",
                              TripleDirName, Ver, "include/c++");
      llvm::sys::path::append(CppIncludeBases.emplace_back(Base), "usr/include",
                              TripleDirName, "c++", Ver);
    }
    for (auto &CppIncludeBase : CppIncludeBases) {
      if (VFS.exists(CppIncludeBase)) {
